...
def main() -> None:
    ...
    parser.add_argument("--question_file", type=str, default="./data/video_instruction_tuning/sqa/llava3d_sqa3d_test_question.json")
    parser.add_argument("--gt_file", type=str, default="./data/video_instruction_tuning/sqa/llava3d_sqa3d_test_answer.json")
    parser.add_argument("--pred_save", type=str, default="./sqa3d_predictions.jsonl")
    ...
    def generate_with_fallback(
        ...
        try:
            ...
            err = f"sqa3d_eval_error: {type(exc).__name__}: {err_msg}"
            if len(batch_rows) > 1:
                if device.type == "cuda" and _is_cuda_oom(exc):
                    torch.cuda.empty_cache()
                if rank == 0 and split_events < 3:
                    print(
                        f"[SQA3D Eval] Batch size {len(batch_rows)} failed ({type(exc).__name__}: {err_msg}); "
    ...
    for start in range(0, len(indices), eval_batch_size):
        ...
        for idx in batch_indices:
            ...
            try:
                ...
                err = f"sqa3d_eval_error: {type(exc).__name__}: {_short_exc(exc)}"
    ...
    if rank == 0:
        ...
        if errors:
            ...
            with open(err_path, "w") as f:
                for row in errors:
                    f.write(json.dumps(row) + "\n")
            print(f"[SQA3D Eval] {len(errors)}/{len(aligned_preds)} samples failed. See: {err_path}", file=sys.stderr)

        metrics: Dict[str, float] = {}
        try:
            evaluator_path = os.path.join(PROJECT_ROOT, "tools", "3d", "sqa3d", "sqa3d_evaluator.py")
            evaluator = _load_module_from_file("sqa3d_evaluator", evaluator_path)
            calc_sqa3d_score = getattr(evaluator, "calc_sqa3d_score", None)
            if not callable(calc_sqa3d_score):
                raise AttributeError("SQA3D evaluator missing required symbol calc_sqa3d_score.")
            ...
            scores_raw = calc_sqa3d_score(aligned_preds, aligned_gts)
            metrics = {str(k): float(v) for k, v in scores_raw.items()}
        except Exception as exc:
            print(f"[SQA3D Eval] Metrics unavailable: {exc}", file=sys.stderr)

        metrics_out = {"sqa3d_total": float(len(aligned_preds))}
