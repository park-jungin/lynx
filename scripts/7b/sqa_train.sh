...
OUTPUT_DIR=${OUTPUT_DIR:-./checkpoints/LynX_$MODEL_SIZE/3d/sqa/train}
DEEPSPEED_CONFIG=${DEEPSPEED_CONFIG:-./scripts/zero2.json}
MASTER_PORT=${MASTER_PORT:-60110}
...
deepspeed --num_gpus 3 --master_port "$MASTER_PORT" train_lynx.py 3d \
  ...
  --train_annotation_file ./data/video_instruction_tuning/sqa/SQA_train_instruct.json \
  --color_subdir color \
  --depth_subdir depth \
