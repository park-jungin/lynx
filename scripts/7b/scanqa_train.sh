...
OUTPUT_DIR=${OUTPUT_DIR:-./checkpoints/LynX_$MODEL_SIZE/3d/scanqa/train}
...
deepspeed --num_gpus 3 --master_port "$MASTER_PORT" train_lynx.py 3d \
  ...
  --train_annotation_file ./data/video_instruction_tuning/scannet/scanqa_train_instruct.json \
  --color_subdir color \
  --depth_subdir depth \
