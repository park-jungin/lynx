...
LLM_ADAPTER_PATH=${LLM_ADAPTER_PATH:-./checkpoints/LynX_$MODEL_SIZE/3d/scanqa/sft}

OUTPUT_DIR=${OUTPUT_DIR:-./downstream_evaluation/3d}
PRED_SAVE=${PRED_SAVE:-$OUTPUT_DIR/scanqa_eval/scanqa_predictions_$MODEL_SIZE.jsonl}

DEEPSPEED_CONFIG=${DEEPSPEED_CONFIG:-./scripts/zero2.json}
MASTER_PORT=${MASTER_PORT:-60140}
...
DEPTH_CLIP_MAX_MM=${DEPTH_CLIP_MAX_MM:-4000.0}
DEPTH_NORMALS_FRAME=${DEPTH_NORMALS_FRAME:-world}
FRAME_SAMPLING=${FRAME_SAMPLING:-pose}
POSE_SUBDIR=${POSE_SUBDIR:-pose}
POSE_MATRIX_TYPE=${POSE_MATRIX_TYPE:-c2w}

EVAL_BATCH_SIZE=${EVAL_BATCH_SIZE:-32}
SCANQA_MAX_SAMPLES=${SCANQA_MAX_SAMPLES:-}
...
if missing:
    print(f"[WARN] ScanQA metrics need: {', '.join(missing)} (pip install pycocoevalcap).")
...
deepspeed --num_gpus 3 --master_port "$MASTER_PORT" scripts/scanqa_eval.py \
  --model_name_or_path "$MODEL_NAME_OR_PATH" \
  --local_files_only \
  --vision_adapter_path "$VISION_ADAPTER_PATH" \
  --llm_adapter_path "$LLM_ADAPTER_PATH" \
  --question_file ./data/video_instruction_tuning/scannet/llava-3d-scanqa_val_question.json \
  --gt_file ./data/video_instruction_tuning/scannet/llava3d_scanqa_val_answer.json \
  ...
  --pred_save "$PRED_SAVE" \
  ${SCANQA_MAX_SAMPLES:+--max_samples "$SCANQA_MAX_SAMPLES"} \
  --use_flash_attn \
  --bf16 \
  --tf32
