...
def main() -> None:
    ...
    parser.add_argument(
        "--annotation_file",
        type=str,
        default="./data/video_instruction_tuning/avsd/mock_test_set4DSTC10-AVSD_from_DSTC7_singref.json",
        help="AVSD dialog annotation file used to construct multi-turn prompts (last turn is generated).",
    )
    parser.add_argument("--gt_file", type=str, default="./data/video_instruction_tuning/avsd/avsd_coco_version_test_gt.json")
    parser.add_argument("--video_root", type=str, default="./data/video_instruction_tuning/avsd/Charades_vu17_test")
    parser.add_argument("--pred_save", type=str, default="./avsd_predictions.json")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Minimum number of tokens to generate (helps avoid empty outputs like immediate <|im_end|>).",
    )
    ...
    if _is_peft_adapter_dir(llm_adapter_dir):
        ...
        if rank == 0:
            ...
            print(f"[AVSD Eval] Loaded LLM LoRA from checkpoint={state_path} with llm_r={llm_r} ({infer_tag}).")
    ...
    dialogs = ann.get("dialogs") if isinstance(ann, dict) else None
    if not isinstance(dialogs, list):
        raise TypeError(f"Expected {args.annotation_file} to contain a dict with list under 'dialogs', got {type(dialogs)}")

    samples: List[Dict[str, Any]] = []
    for ele in dialogs:
        if not isinstance(ele, dict):
            continue
        vid = ele.get("image_id")
        dialog = ele.get("dialog")
        if not isinstance(vid, str) or not vid:
            continue
        if not isinstance(dialog, list) or not dialog:
            continue
        samples.append({"image_id": vid, "dialog": dialog})
    ...
    for start in range(0, len(indices), eval_batch_size):
        ...
        for local_i in batch_indices:
            sample = samples[local_i]
            vid = str(sample["image_id"])
            dialog = sample["dialog"]
            video_path = os.path.join(str(args.video_root), f"{vid}.mp4")
            try:
                if not os.path.exists(video_path):
                    raise FileNotFoundError(video_path)

                messages = _build_avsd_messages(dialog)
                ...
                ph = int(pos.item())
                prefix_ids_prompt = prompt_ids[:ph]
                suffix_ids_prompt = prompt_ids[ph + 1 :]
                ...
                batch_rows.append(
                    {
                        "index": local_i,
                        "image_id": vid,
                        "prefix_ids": prefix_ids_prompt,
                        "suffix_ids": suffix_ids_prompt,
                    }
                )
                ...
                if len(msg) > 300:
                    msg = msg[:300] + "..."
                err = f"avsd_eval_error: {type(exc).__name__}: {msg}"
                local_preds.append({"index": local_i, "image_id": vid, "caption": "", "error": err})
            ...
            try:
                ...
                with amp_ctx:
                    ...
                    gen_ids = core_model.generate(
                        ...
                        min_new_tokens=max(0, int(args.min_new_tokens) if args.min_new_tokens is not None else 0),
                        do_sample=False,
                        num_beams=1,
                    ...
                    for i, row in enumerate(batch_rows):
                        ...
                        local_preds.append({"index": row["index"], "image_id": row["image_id"], "caption": pred})
            except Exception as exc:
                msg = str(exc)
                if len(msg) > 300:
                    msg = msg[:300] + "..."
                ...
                for row in batch_rows:
                    local_preds.append({"index": row["index"], "image_id": row["image_id"], "caption": "", "error": err})
    ...
    if rank == 0:
        ...
        results = [{"image_id": ele["image_id"], "caption": ele.get("caption", "")} for ele in merged]
